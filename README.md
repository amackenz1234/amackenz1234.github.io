...
The Cloud Mac is a GitHub-hosted `xcode-27` VM (full macOS 27 RC on Apple Silicon). Start it from a Windows PC in the browser with **Run virtual machine on Windows**, or install the desktop app:

1. Download [`windows/appcompiler-ai-setup.exe`](windows/appcompiler-ai-setup.exe)
2. Run the installer (per-user, `%LOCALAPPDATA%\Programs\appcompiler.ai`)
3. Launch **appcompiler.ai** from the Start Menu
